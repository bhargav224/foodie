...
export const ACTION_TYPES = {
  SEARCH_USERINFOS: 'userInfo/SEARCH_USERINFOS',
...
export default (state: UserInfoState = initialState, action): UserInfoState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_USERINFOS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_USERINFOS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_USERINFOS):
...
const apiSearchUrl = 'api/_search/user-infos';

// Actions

export const getSearchEntities: ICrudSearchAction<IUserInfo> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_USERINFOS,
  payload: axios.get<IUserInfo>(`${apiSearchUrl}?query=${query}`)
});
