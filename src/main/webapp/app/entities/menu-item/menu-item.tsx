...
import { getSearchEntities, getEntities } from './menu-item.reducer';
...
export class MenuItem extends React.Component<IMenuItemProps, IMenuItemState> {
  ...
  render() {
    const { menuItemList, match } = this.props;
    return (
      <div>
            ...
            <tbody>
              {menuItemList.map((menuItem, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{menuItem.active ? 'true' : 'false'}</td>
                  <td>{menuItem.description}</td>
                  <td>{menuItem.imagePath}</td>
                  <td>{menuItem.name}</td>
                  <td>
                    {menuItem.foodCategorie ? (
                      <Link to={`food-categorie/${menuItem.foodCategorie.id}`}>{menuItem.foodCategorie.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>{menuItem.course ? <Link to={`course/${menuItem.course.id}`}>{menuItem.course.id}</Link> : ''}</td>
                  <td>{menuItem.cusine ? <Link to={`cusine/${menuItem.cusine.id}`}>{menuItem.cusine.id}</Link> : ''}</td>
