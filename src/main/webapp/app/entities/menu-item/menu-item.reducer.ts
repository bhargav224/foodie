...
export const ACTION_TYPES = {
  SEARCH_MENUITEMS: 'menuItem/SEARCH_MENUITEMS',
...
export default (state: MenuItemState = initialState, action): MenuItemState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_MENUITEMS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_MENUITEMS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_MENUITEMS):
...
const apiSearchUrl = 'api/_search/menu-items';

// Actions

export const getSearchEntities: ICrudSearchAction<IMenuItem> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_MENUITEMS,
  payload: axios.get<IMenuItem>(`${apiSearchUrl}?query=${query}`)
});
